...
class DevController extends Controller
{
    ...
    {
        $data = [];
        $resources = Warga::orWhere('nama_warga', 'like', '%' . $request->q . '%')->orWhere('nik', 'like', '%' . $request->q . '%')->where('desa_id',getDesaFromUrl()->id)->get();
    ...
    public function agama()
    {
        ...
        foreach ($agamas as $agama) {
            array_push($array, [
                'label' => $agama->nama,
                'data' => [[1, round($agama->wargas()->where('desa_id', getDesaFromUrl()->id)->count() / Warga::where('desa_id', getDesaFromUrl()->id)->get()->count() * 100)]],
    ...
    public function suku()
    {
        ...
        foreach ($sukus as $suku) {
            array_push($array, [
                'label' => $suku->nama,
                'data' => [[1, round($suku->wargas()->where('desa_id', getDesaFromUrl()->id)->count() / Warga::where('desa_id', getDesaFromUrl()->id)->get()->count() * 100)]],
                'color' => '#' . str_pad(dechex(mt_rand(0, 255)), 2, '0', STR_PAD_LEFT) . str_pad(dechex(mt_rand(0, 255)), 2, '0', STR_PAD_LEFT) . str_pad(dechex(mt_rand(0, 255)), 2, '0', STR_PAD_LEFT)
            ]);
        }
        return response()->json($array);
    ...
    public function golongandarah()
    {
        ...
        foreach ($golongandarahs as $golongandarah) {
            array_push($array, [
                ...
                'data' => [[1, round($golongandarah->wargas()->where('desa_id', getDesaFromUrl()->id)->count() / Warga::where('desa_id', getDesaFromUrl()->id)->get()->count() * 100)]],
                'color' => '#' . str_pad(dechex(mt_rand(0, 255)), 2, '0', STR_PAD_LEFT) . str_pad(dechex(mt_rand(0, 255)), 2, '0', STR_PAD_LEFT) . str_pad(dechex(mt_rand(0, 255)), 2, '0', STR_PAD_LEFT)
            ]);
        }
        return response()->json($array);
    ...
    public function statusperkawinan()
    {
        ...
        foreach ($statusperkawinans as $statusperkawinan) {
            array_push($array, [
                ...
                'data' => [[1, round($statusperkawinan->wargas()->where('desa_id', getDesaFromUrl()->id)->count() / Warga::where('desa_id', getDesaFromUrl()->id)->get()->count() * 100)]],
                'color' => '#' . str_pad(dechex(mt_rand(0, 255)), 2, '0', STR_PAD_LEFT) . str_pad(dechex(mt_rand(0, 255)), 2, '0', STR_PAD_LEFT) . str_pad(dechex(mt_rand(0, 255)), 2, '0', STR_PAD_LEFT)
            ]);
        }
        return response()->json($array);
    ...
    public function pendidikan()
    {
        ...
        foreach ($pendidikans as $pendidikan) {
            array_push($array, [
                ...
                'data' => [[1, round($pendidikan->wargas()->where('desa_id', getDesaFromUrl()->id)->count() / Warga::where('desa_id', getDesaFromUrl()->id)->get()->count() * 100)]],
                'color' => '#' . str_pad(dechex(mt_rand(0, 255)), 2, '0', STR_PAD_LEFT) . str_pad(dechex(mt_rand(0, 255)), 2, '0', STR_PAD_LEFT) . str_pad(dechex(mt_rand(0, 255)), 2, '0', STR_PAD_LEFT)
            ]);
        }
        return response()->json($array);
    ...
    public function pekerjaan()
    {
        ...
        foreach ($pekerjaans as $pekerjaan) {
            array_push($array, [
                ...
                'data' => [[1, round($pekerjaan->wargas()->where('desa_id', getDesaFromUrl()->id)->count() / Warga::where('desa_id', getDesaFromUrl()->id)->get()->count() * 100)]],
                'color' => '#' . str_pad(dechex(mt_rand(0, 255)), 2, '0', STR_PAD_LEFT) . str_pad(dechex(mt_rand(0, 255)), 2, '0', STR_PAD_LEFT) . str_pad(dechex(mt_rand(0, 255)), 2, '0', STR_PAD_LEFT)
            ]);
        }
        return response()->json($array);
    ...
    {
        $warga = Warga::where('desa_id', getDesaFromUrl()->id)->get();
        $arra = [];
        $range = range(1, 80);
